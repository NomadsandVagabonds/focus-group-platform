...
CREATE TABLE questions (
  ...
  settings JSONB DEFAULT '{}', -- mandatory, other, display_columns, min_answers, max_answers, etc.
  relevance_logic TEXT,
  order_index INTEGER NOT NULL,
  created_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE subquestions (
  ...
  code TEXT NOT NULL, -- SQ001, SQ002
  label TEXT NOT NULL,
  order_index INTEGER NOT NULL,
  relevance_logic TEXT,
  created_at TIMESTAMPTZ DEFAULT NOW()
...
CREATE TABLE survey_responses (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  survey_id UUID REFERENCES surveys(id) ON DELETE CASCADE,
  participant_id TEXT, -- PROLIFIC_PID or custom ID
  session_id TEXT, -- SESSION_ID from Prolific
  study_id TEXT, -- STUDY_ID from Prolific
  ...
  randomization_seed TEXT -- For reproducible randomization
);
...
CREATE INDEX idx_question_groups_survey ON question_groups(survey_id);
CREATE INDEX idx_questions_group ON questions(group_id);
CREATE INDEX idx_subquestions_question ON subquestions(question_id);
CREATE INDEX idx_answer_options_question ON answer_options(question_id);
CREATE INDEX idx_survey_responses_survey ON survey_responses(survey_id);
CREATE INDEX idx_survey_responses_participant ON survey_responses(participant_id);
CREATE INDEX idx_response_data_response ON response_data(response_id);
CREATE INDEX idx_response_data_question ON response_data(question_id);
...
-- Public read access for active surveys (participants can view)
