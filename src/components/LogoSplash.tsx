...
// Circle data parsed from vectorized SVG
...
const sourceViewBox = { width: 886, height: 886 };

function parsePathsToCircles(pathStr: string) {
    const circles: { cx: number; cy: number; r: number; id: number }[] = [];
...
export default function LogoSplash({ onComplete }: LogoSplashProps) {
    ...
    useEffect(() => {
        ...
        const getTargets = () => {
            ...
            const size = Math.min(vp.width, vp.height) * 0.55;
            const scale = size / sourceViewBox.width;
            const centerX = vp.width / 2;
            const centerY = vp.height / 2;
            const offsetX = centerX - (sourceViewBox.width * scale) / 2;
            const offsetY = centerY - (sourceViewBox.height * scale) / 2;

            return circleData.map((c) => ({
                targetX: c.cx * scale + offsetX,
                targetY: c.cy * scale + offsetY,
                targetR: c.r * scale,
            }));
        };
        ...
        dotsRef.current = circleData.map((c, i) => {
            ...
            return {
                ...
                targetX: targets[i].targetX,
                targetY: targets[i].targetY,
                targetR: targets[i].targetR,
            };
        });
        ...
        dotsRef.current.forEach((d) => {
            ...
            circle.setAttribute('fill', '#9A3324');
            circle.setAttribute('data-id', String(d.id));
            svg.appendChild(circle);
        });

        const startTime = Date.now();
        const explodePhase = 700;
        const floatPhase = 1100;
        const formPhase = 3000;
        const fadeDelay = 500;
        ...
        const tick = () => {
            ...
            dotsRef.current.forEach((d, i) => {
                d.targetX = targets[i].targetX;
                d.targetY = targets[i].targetY;
                d.r = targets[i].targetR;
            });

            let attractionStrength = 0;

            if (elapsed < explodePhase) {
                attractionStrength = 0;
                dotsRef.current.forEach((d) => {
                    d.vx += (Math.random() - 0.5) * 0.8;
                    d.vy += (Math.random() - 0.5) * 0.8;
                });
            } else if (elapsed < floatPhase) {
                attractionStrength = 0.002;
            } else if (elapsed < formPhase) {
                const formProgress = (elapsed - floatPhase) / (formPhase - floatPhase);
                attractionStrength = Math.pow(formProgress, 2) * 0.025;
            } else {
                // Snap to final position
                dotsRef.current.forEach((d, i) => {
                    d.x = targets[i].targetX;
                    d.y = targets[i].targetY;
                });

                // Update DOM
                const circles = svg.querySelectorAll('circle');
                circles.forEach((circle) => {
                    const id = parseInt(circle.getAttribute('data-id') || '0');
            ...
            dotsRef.current.forEach((d) => {
                ...
                d.vx += dx * attractionStrength;
                d.vy += dy * attractionStrength;
                d.vx *= 0.97;
                d.vy *= 0.97;
                d.x += d.vx;
                d.y += d.vy;
                ...
                const margin = 30;
                if (d.x < margin) { d.x = margin; d.vx *= -0.5; }
                if (d.x > vp.width - margin) { d.x = vp.width - margin; d.vx *= -0.5; }
                if (d.y < margin) { d.y = margin; d.vy *= -0.5; }
                if (d.y > vp.height - margin) { d.y = vp.height - margin; d.vy *= -0.5; }
            });
